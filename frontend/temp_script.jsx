        ...
        const { Route, Link, useLocation, useNavigate } = ReactRouterDOM;
        ...
        const WS_URL = API_BASE.replace('http', 'ws') + '/ws/cockpit';
        ...
        const initWebSocket = () => {
            ...
            const host = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' 
                         ? `${window.location.hostname}:${window.location.port || '8085'}`
                         : window.location.hostname;
            
            const wsUrl = `${protocol}//${host}/ws/cockpit`;
            ...
            sovereign_rtdb = {
                ...
                trigger: function(path, data) {
                    if (this.listeners[path]) {
                        const snapshot = { val: () => data, exists: () => !!data };
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    window.dispatchEvent(new CustomEvent('sovereign-packet', { detail: msg }));
                    ...
                    if (type === 'slot_update' || type === 'live_slots') {
                        ...
                        const fullData = (data && data.signals !== undefined) ? data : { signals: data || [], decisions: [], market_context: {}, updated_at: Date.now() / 1000 };
                    ...
                    else if (type === 'system_sync') {
                        if (data.slots) sovereign_rtdb.trigger('live_slots', data.slots);
                        if (data.banca) sovereign_rtdb.trigger('banca_status', data.banca);
                        if (data.system_state) {
                            sovereign_rtdb.trigger('system_state', data.system_state);
                            sovereign_rtdb.trigger('btc_command_center', data.system_state);
                        }
                    }
        ...
        initWebSocket();
        const db = { collection: () => ({ doc: () => ({ get: () => Promise.resolve({ exists: false }) }) }) }; // Firestore Mock
        ...
        const IntelIcon = ({ type, score, icon, label }) => {
            ...
            return (
                <div className="flex flex-col items-center gap-0.5 group relative">
                    <div className="w-6 h-6 flex items-center justify-center rounded-md bg-white/[0.02] border border-white/5 transition-all group-hover:border-green-500/30" 
           ...
           const AICascadeStatus = ({ cascadeInfo }) => {
            ...
            return (
                <div className="p-4 rounded-[2rem] bg-white/[0.02] border border-white/5 space-y-5 relative overflow-hidden group">
                    ...
                    <div className="flex items-center justify-between relative z-10">
                        <div className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center border border-white/10">
                                <span className="material-icons-round text-sm text-white animate-pulse">psychology</span>
                            </div>
                            <div className="flex flex-col">
                                <h3 className="text-[10px] font-black text-white uppercase tracking-[0.2em]">AI Cascade</h3>
                                <span className="text-[7px] text-gray-500 uppercase font-bold tracking-widest">Neural Multi-Model Pipeline</span>
                            </div>
                        </div>
                        <div className="px-2 py-1 rounded-md bg-white/5 border border-white/10">
                            <span className="text-[8px] font-mono font-bold text-gray-400">{requests} <span className="text-[7px] opacity-50">REQS</span></span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 relative z-10">
                        <div className="flex flex-col gap-2">
                            <div className="flex justify-between items-end">
                                <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Active Intelligence</span>
                                <span className="text-[10px] font-black text-white uppercase tracking-tighter truncate max-w-[140px] drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{last_model || 'Standby'}</span>
                            </div>
                            <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden border border-white/[0.03]">
                                <div className="h-full bg-gradient-to-r from-white/20 via-white to-white/20 scanning-bar" style={{ width: '40%' }}></div>
                            </div>
                        </div>
                                ...
                                return (
                                    <div key={i} className="flex items-center justify-between group/row">
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1 h-1 rounded-full ${colorClass} ${isActive ? 'animate-pulse shadow-[0_0_5px_#fff]' : ''}`}></div>
                                            <span className={`text-[8px] font-black uppercase tracking-widest ${isActive ? 'text-gray-300' : 'text-gray-600'} group-hover/row:text-white transition-colors`}>
                                                {modelShortName}
                                            </span>
                                        </div>
                                        <span className={`text-[7px] font-mono font-bold uppercase ${textColor} opacity-60 group-hover/row:opacity-100 transition-opacity`}>
                                            {item.status}
                                        </span>
                                    </div>
                                );
        ...
        const DailyGoalBanner = ({ context, title }) => {
            ...
            return (
                <div className="mb-4 p-4 rounded-2xl bg-gradient-to-br from-green-500/10 to-white/10 border border-green-500/20 overflow-hidden relative group">
        ...
        const AffinityBadge = ({ symbol, rankings }) => {
            ...
            const color = wr >= 65 ? 'text-white border-green-400/30 bg-green-400/10' : 
                         wr >= 55 ? 'text-white border-green-400/30 bg-green-400/10' : 
                    ...
                    <div className="flex justify-between items-start">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className={`text-xs font-bold ${liveTotalPnL >= 0 ? 'text-success' : 'text-orange-400'}`}>
            ...
            return (
                <div className="flex flex-col sticky top-0 z-[100] bg-gray-950/90 backdrop-blur-2xl">
                    ...
                    <div className="px-4 py-3">
                        <div className="premium-card bg-gradient-to-br from-green-500/[0.05] to-transparent p-3 relative overflow-hidden group">
                            ...
                            <div className="flex justify-between items-center mb-3">
                                <div className="flex flex-col">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className={`text-[9px] font-black px-1.5 py-0.5 rounded flex items-center gap-1 ${btc?.btc_variation_24h >= 0 ? 'bg-green-500/10 text-white' : 'bg-red-500/10 text-red-400'}`}>
                                ...
                                <div className="text-right">
                                    <p className={`text-[10px] font-black px-2 py-0.5 rounded-md border ${
                                        unifiedRegime === 'UP' ? 'bg-green-500/10 text-white border-green-500/20' : 
        ...
        const useAICascadeRT = () => {
            const [status, setStatus] = useState(safeJsonParse(localStorage.getItem('ai_cascade_cache'), {
                ...
                requests: 0,
                cascade: [
                    { model: 'Gemini 2.0 Flash', status: 'ACTIVE' },
                    { model: 'Qwen 2.5 VL', status: 'ACTIVE' },
                    { model: 'Llama 3.1 VL', status: 'ACTIVE' },
                    { model: 'Pixtral 12B', status: 'ACTIVE' },
                    { model: 'Gemma 3 IT', status: 'ACTIVE' },
                    { model: 'Gemini Lite', status: 'ACTIVE' }
                ]
            }));
        ...
        const useSlotsRT = (initialData) => {
            ...
            useEffect(() => {
                const handlePacket = (e) => {
                    const message = e.detail;
                    if (message.type === 'live_slots' && message.data) {
                        const val = message.data;
                        let rawSlots = [];
        ...
        const useSystemState = () => {
            ...
            useEffect(() => {
                ...
                if (sovereign_rtdb) {
                    ...
                    } catch (e) { console.error("System State sovereign_rtdb Error:", e); }
                }
        ...
        const IntelligenceFlowCard = () => {
            ...
            const getAgentIcon = (agent) => {
                switch(agent?.toLowerCase()) {
                    case 'captain': return { icon: 'military_tech', color: 'text-white' };
                    case 'vision': return { icon: 'visibility', color: 'text-green-400' };
                    case 'librarian': return { icon: 'menu_book', color: 'text-amber-400' };
                    case 'guardian': return { icon: 'shield', color: 'text-blue-400' };
                    case 'macro': return { icon: 'public', color: 'text-purple-400' };
                    default: return { icon: 'psychology', color: 'text-gray-400' };
                }
            };
            ...
            return (
                <div className="space-y-4">
                    ...
                    <div className="relative glass-card p-4 rounded-3xl border border-white/5 bg-black/20 overflow-hidden">
                        <div className="intel-line"></div>
                        <div className="max-h-[400px] overflow-y-auto no-scrollbar space-y-2 pr-1" ref={containerRef}>
                            {events.map((ev, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="intel-avatar">
                                            <span className={`material-icons-round text-base ${agentCfg.color}`}>{agentCfg.icon}</span>
                                        </div>
                                        <div className="intel-bubble">
                                            <div className="flex justify-between items-start mb-1">
                                                <span className="text-[9px] font-black uppercase tracking-widest text-white/60">{ev.agent}</span>
                                                <span className="text-[7px] font-mono text-gray-600">{new Date(ev.timestamp).toLocaleTimeString()}</span>
                                            </div>
                                            <p className="text-[10px] text-gray-300 leading-tight font-medium">{ev.message}</p>
                                            
                                            {/* Vision Specific Content */}
                                            {ev.payload && ev.payload.image_url && (
                                                <div className="mt-2 group relative">
                                                    ...
                                                    />
                                                    <div className="absolute top-2 right-2 px-2 py-0.5 rounded bg-black/60 backdrop-blur-md border border-white/10 text-[8px] font-black text-white uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity">
                                                        View Full Chart
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {ev.payload && ev.payload.decision && (
                                                <div className="mt-2 flex items-center gap-2">
                                                    <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-tighter ${ev.payload.decision === 'APPROVED' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
                                                        {ev.payload.decision}
                                                    </span>
                                                    {ev.payload.confidence && (
                                                        <span className="text-[8px] font-bold text-gray-500 uppercase">Confidence: {ev.payload.confidence}%</span>
                                                    )}
                                                </div>
                                            )}

                                            {/* Thoughts */}
                                            {ev.payload && ev.payload.thoughts && (
                                                <p className="mt-2 text-[9px] text-gray-500 italic border-l border-white/10 pl-2">
                                                    "{ev.payload.thoughts}"
                                                </p>
                                            )}
                                        </div>
        ...
        const useMarketRadarRT = () => {
            ...
            useEffect(() => {
                ...
                try {
                    ...
                    return () => ref.off('value', onValue);
                } catch (e) { console.error("Radar sovereign_rtdb Error:", e); }
            }, [sovereign_rtdb]);
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-center gap-2 opacity-60">
                                ...
                                <div className="flex items-center gap-1.5 bg-black/30 px-1.5 py-0.5 rounded-md border border-white/5">
                                    <div className={`w-1.5 h-1.5 rounded-full ${pulseStatus === 'ONLINE' && !isStale ? 'bg-green-500 shadow-[0_0_5px_rgba(34,197,94,1)] animate-pulse' : isStale ? 'bg-amber-500' : 'bg-red-500'}`}></div>
                            ...
                            <div className="flex flex-col items-center sm:items-start pl-1">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    {(() => {
                                        ...
                                        if (isTrending && isUp) {
                                            return <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest bg-green-500 text-black shadow-[0_0_10px_rgba(34,197,94,0.5)]">MERCADO EM ALTA</span>;
                ...
                <div key={symbol} className={`relative p-2.5 rounded-xl border transition-all duration-300 flex flex-col items-center justify-center overflow-hidden
                    ...
                    <div className="mt-1.5 w-full flex flex-col gap-1.5 items-center">
                        ...
                        {data.decorrelation && (
                            <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[8px] font-black uppercase tracking-tighter transition-all duration-500
                                ${data.decorrelation.is_active ? 
                                    (data.decorrelation.score >= 80 ? 'bg-white/10 border-white/20 text-white shadow-[0_0_10px_rgba(34,197,94,0.3)] animate-pulse' : 'bg-green-500/10 border-green-500/20 text-white/80') : 
                        ...
                        <div className="flex justify-between w-full px-0.5 mt-0.5">
                            <span className={`text-[7px] font-bold ${data.rsi > 70 ? 'text-red-400' : data.rsi < 30 ? 'text-success' : 'text-gray-500'}`}>
        ...
        const TradingChart = ({ symbol, entryPrice, stopLossPrice, targetPrice, side, pnlFallback, slotId, slotType }) => {
            ...
            useEffect(() => {
                ...
                const connectWs = () => {
                    ...
                    ws.onerror = (e) => {
                        // Silent close for cleaner console
                ...
                const timeout = setTimeout(() => {
                    ...
                    if (symbol && ne > 0) {
                        ...
                        if (isSniper) {
                            ...
                            try {
                                linesRef.current.push(seriesRef.current.createPriceLine({
                                    price: target150, color: '#22c55e', lineWidth: 2, lineStyle: LightweightCharts.LineStyle.Dashed,
            ...
            return (
                <div className="w-full h-full relative group">
                    ...
                    <div className="absolute top-4 right-4 z-10 flex flex-col items-end pointer-events-none">
                        ...
                        <div className={`mt-1 px-2 py-0.5 rounded text-[9px] font-bold flex items-center gap-1 ${trendData.trend === 'bullish' ? 'bg-white/10 text-white border border-green-500/30' :
        ...
        const useBybitTickers = (activeSymbols) => {
            ...
            useEffect(() => {
                ...
                const connect = () => {
                    ...
                    ws.onerror = (err) => {
                        // V110.201: Silent error handling for Bybit Stream
                        if (ws.readyState === WebSocket.OPEN) ws.close();
                    };
                };
                        ...
                        <div className="fixed inset-0 z-[100] flex">
                            ...
                            <div className="relative w-64 h-full v5-bg-deep border-r v5-border-subtle shadow-2xl flex flex-col p-6 animate-in slide-in-from-left duration-300 glass-jarvis">
                                ...
                                <div className="mt-auto pt-8 border-t v5-border-subtle">
                                    <p className="text-[10px] text-gray-600 uppercase tracking-[0.3em] text-center font-bold">Protocol JARVIS V110.182.9</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <footer className="flex-shrink-0 px-4 pt-2 pb-24 v5-bg-deep/80 backdrop-blur-3xl border-none relative z-40">
                        <div className="max-w-3xl mx-auto">
                            ...
                            <div className="flex justify-between items-center mt-4 px-4 opacity-40">
                                ...
                                <p className="text-[9px] text-gray-500 font-mono uppercase tracking-[0.3em] font-bold">Neural Link v110.182.9</p>
                            </div>
                        </div>
                    </footer>
                </div>
        ...
        const SlotCard = ({ s, tickers, marketRadar, librarianIntel, compact = true, isSelected = false, onClick = null }) => {
            ...
            return (
                ...
                >
                    {/* [V4.0] Respiro Badge - Dynamic Visibility */}
                    {(() => {
                        ...
                        if (isRespirando) {
                            return (
                                <div className="absolute top-2 left-2 z-30 animate-pulse">
                                    <div className="bg-success/20 border border-success/40 text-success text-[8px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 shadow-[0_0_10px_rgba(34,197,94,0.2)]">
                    ...
                    {/* [V110.506] Strategy Badge - Dynamic Injection */}
                    <div className={`strategy-badge-elite !rounded-bl-3xl ${
                        (s.strategy_label === 'ABCD' || s.type === 'ESTRATEGIA_N') 
                        ? 'bg-white shadow-[0_0_15px_rgba(255,255,255,0.3)]' 
                        : (s.is_spring_strike ? 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.4)]' : 'bg-white')}`}>
                        <span className="px-1 text-black font-black">
                            {s.strategy_label || 
                             (s.type === 'ESTRATEGIA_N' ? 'ABCD' : 
                             (s.is_spring_strike ? 'MOLA' : 
                             (s.is_shadow_strike ? 'SHADOW' :
                             (s.strategy_label || 'BLITZ 30M'))))}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-1">
                        ...
                        <div className="flex flex-col items-end">
                            <span className={`text-3xl font-black font-mono leading-none ${isProfit ? 'text-success' : 'text-danger'}`} style={{ textShadow: isProfit ? '0 0 15px rgba(34, 197, 94, 0.3)' : '0 0 15px rgba(239, 68, 68, 0.2)' }}>
                                {isProfit ? '+' : ''}{displayPnl.toFixed(1)}%
                            </span>
                            <span className={`text-[11px] font-bold font-mono mt-1 ${isProfit ? 'text-success' : 'text-danger'} opacity-80`}>
        ...
        const GridChartItem = ({ 
            ...
            const [telemetry, setTelemetry] = React.useState({ rsi: null, adx: null, volatility: null, cvd: null });
            const [settings, setSettings] = React.useState(() => {
                try { 
                    const saved = localStorage.getItem('eagle_vision_settings');
                    return saved ? JSON.parse(saved) : { volume: true, sma: true, adx: true, cvd: true, sentinel: true };
                } catch(e) { return { volume: true, sma: true, adx: true, cvd: true, sentinel: true }; }
            });
            ...
            React.useEffect(() => {
                if (candlestickSeries.current && historicalData.candles.length > 0) {
                    ...
                    if (chartInstance.current.volumeSeries) chartInstance.current.volumeSeries.applyOptions({ visible: settings.volume });
                    if (sma8SeriesRef.current) sma8SeriesRef.current.applyOptions({ visible: settings.sma && eagleMode });
                    if (sma21SeriesRef.current) sma21SeriesRef.current.applyOptions({ visible: settings.sma && eagleMode });
                }
            ...
            React.useEffect(() => {
                ...
                if (points && points.entry > 0) {
                    ...
                    // [V110.182.6] Entry Candle Marker
                    if (historicalData.candles.length > 0) {
                        const entryPrice = points.entry;
                        // Find closest candle to entry price (simplified proxy for entry candle if exact time unknown)
                        const entryCandle = historicalData.candles.find(c => Math.abs(c.close - entryPrice) / entryPrice < 0.001);
                        if (entryCandle) {
                            candlestickSeries.current.setMarkers([{
                                time: entryCandle.time,
                                position: points.side === 'LONG' || points.side === 'Buy' ? 'belowBar' : 'aboveBar',
                                color: '#fff',
                                shape: 'arrowUp',
                                text: 'ENTRY'
                            }]);
            ...
            }, [slots, localSymbol, focusedAsset, historicalData]);
            ...
            return (
                <div className="flex flex-col h-full border border-white/5 bg-black/40 hover:border-white/20 transition-all relative group overflow-hidden rounded-[1.5rem] shadow-2xl backdrop-blur-xl">
                    {settings.sentinel && <div className="scanning-effect"></div>}
                    
                    {/* Header HUD: Symbol & Interval */}
                    <div className="absolute top-4 left-5 z-20 flex flex-col gap-1 pointer-events-none">
                        <div className="flex items-center gap-3">
                            <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                            ...
                            {focusedAsset && focusedAsset.symbol.replace('.P','') === localSymbol && (
                                <div className="px-2 py-0.5 rounded bg-primary/20 border border-primary/40 text-[8px] font-black text-primary uppercase tracking-widest animate-pulse">
                                    TOCAIA ATIVA
                                </div>
                            )}
                        </div>
                        
                        {/* [V110.182.9] Telemetry moved to Top-Left for Volume Visibility */}
                        <div className="flex gap-4 mt-2 opacity-60 group-hover:opacity-100 transition-opacity">
                            <div className="flex flex-col">
                                <span className="text-[7px] font-black text-gray-500 uppercase">RSI</span>
                            ...
                            {settings.cvd && (
                                <div className="flex flex-col border-l border-white/10 pl-3">
                                    <span className="text-[7px] font-black text-gray-500 uppercase">CVD</span>
                                    <span className={`text-[10px] font-black font-mono ${telemetry.cvd >= 0 ? 'text-success' : 'text-danger'}`}>
                                        {telemetry.cvd ? (telemetry.cvd / 1000).toFixed(1) + 'K' : '--'}
                                    </span>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Chart Controls: Moved away from Right Price Scale */}
                    <div className="absolute top-4 right-[65px] z-40 flex items-center gap-2 opacity-40 group-hover:opacity-100 transition-all">
                        <div className="relative group/sel">
                            ...
                            <span className="material-icons-round absolute right-2 top-1/2 -translate-y-1/2 text-[14px] text-gray-500 group-hover/sel:text-primary pointer-events-none">expand_more</span>
                        </div>
                        <button 
                        ...
                        >
                            <span className="material-icons-round text-[18px]">{eagleMode ? 'auto_awesome' : 'visibility_off'}</span>
                        </button>
                        <button 
                    ...
                    {tooltip && (
                        ...
                        >
                            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-[9px] font-mono">
                                <span className="text-gray-500 uppercase">O:</span> <span className="text-white font-bold">{tooltip.open.toFixed(2)}</span>
                                <span className="text-gray-500 uppercase">H:</span> <span className="text-white font-bold">{tooltip.high.toFixed(2)}</span>
                                <span className="text-gray-500 uppercase">L:</span> <span className="text-white font-bold">{tooltip.low.toFixed(2)}</span>
                                <span className="text-gray-500 uppercase">C:</span> <span className="text-white font-bold">{tooltip.close.toFixed(2)}</span>
                            </div>
                        </div>
                    )}

                    {/* Settings Panel Overlay: Repositioned to align with control group */}
                    {showSettings && (
                        <div className="absolute top-14 right-[65px] z-[60] bg-black/95 backdrop-blur-3xl border border-white/10 p-3 rounded-2xl shadow-3xl flex flex-col gap-2 min-w-[140px] animate-triumph">
                            <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-1">Grid Config</span>
                            {[
                                { key: 'volume', label: 'Volume Bars', icon: 'bar_chart' },
                                { key: 'sma', label: 'SMA 8/21', icon: 'show_chart' },
                                { key: 'adx', label: 'ADX Index', icon: 'trending_up' },
                                { key: 'cvd', label: 'CVD Delta', icon: 'waves' },
                                { key: 'sentinel', label: 'Flow Sentinel', icon: 'radar' }
                            ].map(opt => (
                                <button 
                                    key={opt.key}
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        <span className="material-icons-round text-[14px] text-gray-400 group-hover/opt:text-white">{opt.icon}</span>
                                        <span className="text-[10px] font-bold text-gray-300">{opt.label}</span>
                                    </div>
                                    <div className={`w-3 h-3 rounded-sm border ${settings[opt.key] ? 'bg-primary border-primary' : 'border-white/20'}`}>
                                        {settings[opt.key] && <span className="material-icons-round text-[10px] text-black font-black">check</span>}
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    {/* Eagle Mode Status Overlay: Pushed down slightly to not overlap Telemetry */}
                    {eagleMode && historicalData.candles.length > 0 && (
                        <div className="absolute top-[85px] left-5 z-20 flex flex-col gap-1 pointer-events-none animate-triumph">
                            <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md border border-white/10 px-2.5 py-1.5 rounded-xl">
                                <div className={`w-2 h-2 rounded-full ${historicalData.candles[historicalData.candles.length-1].close > historicalData.candles[historicalData.candles.length-1].open ? 'bg-success shadow-[0_0_12px_#22c55e]' : 'bg-danger shadow-[0_0_12px_#ef4444]'}`}></div>
                                <span className="text-[8px] font-black text-white uppercase tracking-widest">
                                    {historicalData.candles[historicalData.candles.length-1].close > historicalData.candles[historicalData.candles.length-1].open ? 'Market Sentiment: Bullish' : 'Market Sentiment: Bearish'}
                                </span>
                            </div>
                            <span className="text-[7px] font-bold text-white/20 uppercase tracking-[0.4em] ml-1">Eagle Vision Protocol v110.182.9</span>
                        </div>
                    )}
            ...
            return (
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <span className={`text-[8px] font-black uppercase px-2 py-1 rounded border tracking-widest ${isScanning ? 'bg-amber-500/10 text-amber-400 border-amber-500/20 animate-pulse' : 'bg-green-500/10 text-white border-green-500/20'}`}>
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div className="flex items-center gap-3 bg-white/[0.02] px-4 py-1.5 rounded-xl border border-white/5 hover:border-accent/20 transition-all">
                             <div className={`w-7 h-7 rounded-full flex items-center justify-center border transition-all duration-1000 ${isStabilizing ? 'bg-amber-500/10 border-amber-500/20' : 'bg-accent/5 border-accent/10'}`}>
                                <span className={`material-icons-round text-sm ${isStabilizing ? 'text-amber-500 animate-spin' : 'text-success drop-shadow-[0_0_5px_rgba(34,197,94,0.4)]'}`}>
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <div className="flex flex-col border-l border-white/5 pl-6 justify-center">
                                ...
                                <span className="flex items-center gap-2 text-white font-black uppercase tracking-tighter leading-none">
                                    ...
                                    {protocolLabel || 'Sniper V110.182.8'}
                                </span>
                            </div>
                        </div>
                    </div>
        ...
        const DesktopDashboard = ({
            ...
            unifiedRegime, librarianIntel, ai_cascade
        }) => {
                ...
                <div className="desktop-command-grid h-screen overflow-hidden flex bg-[#050505] text-white">
                    ...
                    <aside className="w-[300px] border-none flex flex-col bg-black/20 backdrop-blur-md z-20 overflow-y-auto no-scrollbar">
                        <div className="p-6 space-y-8">
                            {/* 1. OBSERVATORY BUTTON [V5.0] */}
                            <section>
                                <a
                                    href="/observatory" target="_blank" rel="noopener noreferrer"
                                    className="w-full flex items-center gap-3 p-4 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-white/20 hover:bg-white/[0.06] transition-all group cursor-pointer"
                                >
                                    <div className="w-8 h-8 rounded-xl bg-green-500/10 border border-green-500/20 flex items-center justify-center flex-shrink-0 group-hover:border-green-500/40 transition-all">
                                        <span className="material-icons-round text-[16px] text-green-400">satellite_alt</span>
                                    </div>
                                    <div className="flex flex-col items-start text-left">
                                        <span className="text-[11px] font-black text-white uppercase tracking-widest">Observatory</span>
                                        <span className="text-[8px] text-gray-500 font-mono">40 Elite Pairs · Visual HQ</span>
                                    </div>
                                    <span className="ml-auto material-icons-round text-gray-600 text-sm group-hover:text-gray-400 transition-colors">open_in_new</span>
                                </a>
                            </section>

                            {/* 2. AI CASCADE MONITOR [V4.2.1] */}
                            <section>
                                <AICascadeStatus cascadeInfo={ai_cascade || {}} />
                            </section>

                            {/* 3. COLLECTIVE INTELLIGENCE FLOW [V1.0] */}
                            <section>
                                <IntelligenceFlowCard />
                            </section>

                            {/* 3. INTELLIGENCE LIBRARIAN (Expansive) */}
                            ...
                            <section className="space-y-4">
                                ...
                                <div className={`p-6 glass-premium border ${isDrawdown ? 'border-amber-500/30' : 'border-white/10'} relative overflow-hidden group`}>
                                    ...
                                    <div className="relative z-10 space-y-1">
                                        <span className={`text-4xl font-black font-display tracking-tighter ${liveTotalProfit >= 0 ? 'text-success' : 'text-danger'}`} style={{ textShadow: liveTotalProfit >= 0 ? '0 0 15px rgba(34, 197, 94, 0.3)' : '0 0 15px rgba(239,68,68,0.2)' }}>
                                            ${liveEquity.toFixed(2)}
                                        </span>
                                        <div className="flex items-center gap-2">
                                            <span className={`text-xs font-bold ${liveTotalProfit >= 0 ? 'text-success' : 'text-danger'}`}>
                                    ...
                                    <div className="mt-6 w-full bg-black/50 h-2 rounded-full overflow-hidden border border-white/5">
                                        <div className={`h-full bg-gradient-to-r from-accent to-white shadow-[0_0_10px_rgba(34,197,94,0.3)] transition-all duration-1000`} style={{ width: `${hullIntegrityPct}%` }}></div>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="space-y-4 pt-4 border-t border-white/5">
                                ...
                                <div className="space-y-2.5">
                                    {Array.isArray(radarSignals) && radarSignals.slice(0, 5).map((sig, idx) => {
                                        const sLabel = sig.type === 'ESTRATEGIA_N' ? 'ABCD' : 
                                                      (sig.is_spring_strike ? 'MOLA' : 
                                                      (sig.is_shadow_strike || sig.indicators?.pattern === 'shadow_needle' ? 'SHADOW' : 
                                                      (sig.is_123 || sig.pattern === '123_pattern' ? '1-2-3' : '')));
                                        return (
                                            <div key={idx} className="p-3 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center justify-between group hover:border-gray-500/30 transition-all">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {sLabel && (
                                                        <span className={`strategy-badge ${sLabel.toLowerCase().replace('1-2-3', 'blitz')}`}>
                                                            {sLabel}
                                                        </span>
                                                    )}
                                            ...
                                            {historyLogs && historyLogs.slice(0, 5).map((log, i) => (
                                                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                                                    ...
                                                    <td className="p-3 text-right">
                                                        <div className={`font-black font-mono ${log.pnl >= 0 ? 'text-success' : 'text-red-400'}`}>
...
// --- Component: Page10D (Centralized HQ) ---
        const Page10D = () => {
            ...
            const banca = useBancaRT(safeJsonParse(localStorage.getItem('banca_cache'), { saldo_total: 100, configured_balance: 100, lucro_total_acumulado: 0, lucro_ciclo: 0 }));
            ...
            const protocolLabel = btcCommandStatus?.protocol || (systemState && systemState.protocol) || "Sniper V110.182.9";
                                                        ...
                                                        <div className="flex flex-col">
                                                            <div className="flex items-center gap-2">
                                                                <span className="text-sm font-bold text-white uppercase">{sig.symbol.replace('.P','')}</span>
                                                                 {/* [V110.506] Strategy Badge Unified Logic */}
                                                                 {(() => {
                                                                     const sLabel = sig.type === 'ESTRATEGIA_N' ? 'ABCD' : 
                                                                                  (sig.is_spring_strike ? "MOLA" : 
                                                                                  (sig.is_shadow_strike || sig.indicators?.pattern === "shadow_needle" ? "SHADOW" : 
                                                                                  (sig.is_123 || sig.pattern === "123_pattern" ? "1-2-3" : 
                                                                                  (sig.strategy_label || "BLITZ")))));
                                                                     return (
                                                                         <span className={`strategy-badge ${sLabel.toLowerCase().replace('1-2-3', 'blitz')}`}>
                                                                             {sLabel}
                                                                         </span>
                                        ...
                                        historyLogs.map((log, i) => {
                                            ...
                                            return (
                                                <div key={i} onClick={() => setSelectedHistoryLog(log)} className="flex justify-between items-center p-3 rounded-xl bg-white/[0.02] border border-white/5 active:bg-white/5 transition-colors">
                                                    ...
                                                    <div className="text-right">
                                                        <span className={`text-xs font-mono font-black ${isProfit ? 'text-success' : 'text-red-400'}`}>
                                                            {isProfit ? '+' : '-'}${Math.abs(log.pnl || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                        </span>
                    ...
                    {selectedHistoryLog && (() => {
                        ...
                        return (
                            <div className="fixed inset-0 z-[120] bg-black/90 backdrop-blur-2xl flex items-center justify-center p-6" onClick={() => setSelectedHistoryLog(null)}>
                                ...
                                >
                                    {/* Header de Triunfo */}
                                    <div className={`p-6 border-b border-white/5 flex items-center justify-between ${isAstronomical ? 'bg-green-900/20' : isProfit ? 'bg-green-900/10' : 'bg-white/5'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${isAstronomical ? 'bg-white/20 border-white/30' : 'bg-white/10 border-green-500/30'}`}>
                                    ...
                                    <div className="p-6 overflow-y-auto custom-scrollbar flex flex-col gap-6">
                                        ...
                                        <div className="relative group p-6 rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col items-center justify-center text-center overflow-hidden">
                                            ...
                                            <div className="flex gap-2 mt-4">
                                                {finalRoi >= 20 && <span className="px-2 py-0.5 rounded-full bg-white/10 border border-green-500/30 text-[8px] font-black text-white uppercase tracking-widest">🌊 WAVE</span>}
                                                {finalRoi >= 50 && <span className="px-2 py-0.5 rounded-full bg-white/10 border border-green-500/30 text-[8px] font-black text-white uppercase tracking-widest">⚡ VOLT</span>}
                                                {finalRoi >= 100 && <span className="px-2 py-0.5 rounded-full bg-white/10 border border-green-500/30 text-[8px] font-black text-white uppercase tracking-widest">🚀 ROCKET</span>}
